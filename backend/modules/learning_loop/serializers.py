...
class SharedLearningStateAggregateSerializer(serializers.Serializer):
    ...
    activeProject = ActiveProjectSerializer(source="active_project", allow_null=True)
    mentorIntervention = MentorInterventionSerializer(source="mentor_intervention", allow_null=True)
    parentBridge = ParentBridgeSerializer(source="parent_bridge", allow_null=True)
