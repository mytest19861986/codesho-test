...
export function DashboardScreen({ model = dashboardFixture, state = "ready" }: { model?: DashboardModel; state?: DashboardViewState }) {
  return <AppShell activeItemId="dashboard" brand={<Brand />} drawerCloseLabel="بستن منو" menuButtonLabel="باز کردن منوی داشبورد" bottomNavigationItems={navigationItems} navigationItems={navigationItems} navigationLabel="ناوبری داشبورد" tone="learner" profileSlot={<span className={styles.profile}>آ</span>}><div className={styles.page}><DashboardContent model={model} state={state} /></div></AppShell>;
}
