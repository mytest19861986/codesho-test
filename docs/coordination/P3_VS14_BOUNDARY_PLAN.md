# P3-VS14 Boundary Plan, DDL & Proof Matrix (v1.4)
...
| `ACTIVE` | Complete Goal | `ACHIEVED` | Student / Mentor | `completed_at = clock_timestamp()`; All action plans verified |
| `ACTIVE` | Pause Goal | `PAUSED` | Student | Free active slot in domain |
| `PAUSED` | Resume Goal | `ACTIVE` | Student | Re-check domain active uniqueness & total <= 5 |
| `ACTIVE` | Supersede Goal | `SUPERSEDED` | Student | Superseded by newer refined goal in same domain |
| `ACTIVE` / `PAUSED` | Abandon Goal | `ARCHIVED` | Student | Soft-archived, non-punitive |
| *Illegal* | Jump from Draft to Achieved | *REJECTED* | Any | Direct jump `DRAFT -> ACHIEVED` strictly forbidden by DB check |
...
| `PRESENTED` | Student Dismisses Suggestion | `DISMISSED` | Student | Marked dismissed with timestamp |
...
## 3. Detailed Negative Proof Matrix (N1 – N25)
...
| **N12** | B4 Invariant Proof | Delete `platform_tenant_tenant` (Tenant Wipe) | Cascades across all 6 tables cleanly without RESTRICT lockup (DB PASS) |
...
| **N14** | M2 FSM Illegal Jump | Transition `StudentLearningGoal` directly from `DRAFT` to `ACHIEVED` | Constraint `chk_goal_completion_consistency` violation -> DB REJECT |
...
| **N21** | F5 / M3 PII JSONB | Inject `{"credit_card": "1234..."}` into `evidence_context` | Constraint `chk_suggestion_evidence_no_pii` (`?\|`) violation -> DB REJECT |
...
## 4. Canonical DDL (PostgreSQL 17)
The DDL is committed in `docs/architecture/p3_vs14_schema_ddl.sql` with deferred target foreign keys for `ReflectionAuditLog` to eliminate tenant cascade order sensitivity.
