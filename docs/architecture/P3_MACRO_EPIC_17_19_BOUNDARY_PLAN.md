# P3-MACRO-EPIC-17-19 Boundary Plan & Architecture Specification (v1.1-CANONICAL)
...
- Response-Record Identity: Fully addresses GLM v1.1 Audit findings (B1, B2, M1-M8).

---

## 2. Integrated Slices Architecture & Domain Boundaries

### 2.1. Prerequisites & Shared Infrastructure Foundations
...
- Audit Discipline: Dedicated append-only `learning_mentoroperationsauditlog` with `DEFERRABLE INITIALLY DEFERRED` foreign keys protecting historical audit records from parent deletion.

### 2.2. P3-VS17: Mentor Caseload & Support Operations
- **Core Models**:
  - `MentorCaseloadAssignment`: Mapping of student to mentor with active/inactive statuses, capacity weight, and unassignment reasoning.
  - `SupportQueueItem`: Operational queue of mentor actions (due date, urgency level, queue status).
- **Invariants**:
...
- **Core Models**:
  - `LearningCheckIn`: Structured check-in sessions with status, scheduled start, actual timing, and student voluntary acknowledgement.
  - `FollowUpCommitment`: Mutual commitments with owner role (`MENTOR` or `STUDENT`), due date, and completion status.
- **FSM States**:
  - Check-in: `SCHEDULED` -> `IN_PROGRESS` -> `COMPLETED` / `RESCHEDULED` / `CANCELLED`
  - Self-referencing composite FK: `fk_checkin_rescheduled_from` with `ON DELETE SET NULL (rescheduled_from_id)`.
- **Invariants**:
...
2. **Composite Foreign Keys & SET NULL Discipline**:
...
3. **Session Protocol & GUC**:
   - Queries executed strictly within `transaction.atomic()` after `SET LOCAL "app.current_tenant" = %s`.
4. **Comprehensive 21-Key PII Blacklist & Regex Filtering**:
   ...
   - Free-text fields (`notes`, `resolution_notes`, `title`) guarded by comprehensive regex anti-PII constraints.
5. **Audit Log Target Integrity**:
   - Exact 5-way XOR constraint: `num_nonnulls(target_caseload_id, target_queue_item_id, target_checkin_id, target_commitment_id, target_aggregate_id) = 1`.
...
| **N19** | PII Blacklist | Injecting phone number regex pattern in Check-in notes | CheckConstraint violation (`chk_checkin_notes_no_pii`) |
| **N20** | PII Blacklist | Injecting IBAN in Queue resolution notes | CheckConstraint violation (`chk_supportqueue_resolution_no_pii`) |
...
| **N25** | Concurrency | Concurrent status updates on same Queue item | Managed via `pg_advisory_xact_lock` without race condition |
